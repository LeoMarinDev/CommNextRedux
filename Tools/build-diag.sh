#!/usr/bin/env bash
#
# Builds the temporary CommNextRedux UI diagnostic mod (Tools/diag/DiagPlugin.cs) into
# Deploy/CommNextReduxDiag/CommNextReduxDiag.dll, ready to copy to $KSP2/Mods/CommNextReduxDiag/.
#
# It is NOT part of CommNextRedux.dll and is never shipped. It exists so a single game launch can
...
OUT_DIR="$REPO_ROOT/Deploy/CommNextReduxDiag"
OUT_DLL="$OUT_DIR/CommNextReduxDiag.dll"
...
# ReduxLib, and compiling against the complete set keeps the binding identical to CommNextRedux.dll's.
...
{
  "spec": "2.0",
  "mod_id": "CommNextReduxDiag",
  "name": "CommNext Redux UI Diagnostic",
  "author": "local",
  "description": "Temporary diagnostic probe for AssetBundle/UI Toolkit font compatibility. Remove after use.",
  "source": "",
  "version": "1.0.0",
  "version_check": "",
  "ksp2_version": { "min": "0.2.8.5", "max": "*" },
  "dependencies": [
    { "id": "SpaceWarp2", "version": { "min": "2.0.0", "max": "*" } }
  ],
  "conflicts": [],
  "main_assembly": "CommNextReduxDiag.dll"
}
EOF

mkdir -p "$OUT_DIR/variants"
log "Wrote $OUT_DLL ($(stat -c %s "$OUT_DLL") bytes)"
log "Payload: $OUT_DIR (swinfo.json + CommNextReduxDiag.dll + variants/)"
log ""
log "Deploy with:"
log "  cp -f \"$OUT_DLL\" \"\$KSP2/Mods/CommNextReduxDiag/\""
